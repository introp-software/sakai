...
// */
// public class SkinnableOauth2Login extends HttpServlet implements Login {
//
// private static final long serialVersionUID = 1L;
//
// /** Our log (commons). */
// private static Log log = LogFactory.getLog(SkinnableOauth2Login.class);
//
// /** Session attribute used to store a message between steps. */
// protected static final String ATTR_MSG = "notify";
//
// /**
// * Session attribute set and shared with ContainerLoginTool: URL for
// * redirecting back here.
// */
// public static final String ATTR_RETURN_URL = "sakai.login.return.url";
//
// /**
// * Session attribute set and shared with ContainerLoginTool: if set we have
// * failed container and need to check internal.
// */
// public static final String ATTR_CONTAINER_CHECKED =
// "sakai.login.container.checked";
//
// /**
// * Session attribute to show that session was successfully authenticated
// * through the container.
// */
// public static final String ATTR_CONTAINER_SUCCESS =
// "sakai.login.container.success";
//
// // Services are transient because the class is serializable but the services
// // aren't
...
// }
//
// // get the parts (the first will be "", second will be "login" or
// // "logout")
// String[] parts = option.split("/");
//
// if (parts[1].equals("logout")) {
// // get the session info complete needs, since the logout will
// // invalidate and clear the session
// String containerLogoutUrl =
// serverConfigurationService.getString("login.container.logout.url", null);
// String containerLogout =
// getServletConfig().getInitParameter("container-logout");
// if (containerLogoutUrl != null &&
// session.getAttribute(ATTR_CONTAINER_SUCCESS) != null
...
// }
//
// // SAK-29092 if an auth is specified in the URL, skip any other checks
// // and go straight to it
// String authPreferred = req.getParameter("auth");
// log.debug("authPreferred: " + authPreferred);
//
// if (StringUtils.equalsIgnoreCase(authPreferred,
// AuthChoices.XLOGIN.toString())) {
...
// serverConfigurationService.getBoolean("container.login", false);
...
//
// String containerCheckPath =
// this.getServletConfig().getInitParameter("container");
...
// * data is already URL encoded. Had to @SuppressWarnings the
// * entire method.
// */
//
// // SAK-21498 choice page for selecting auth sources
// showAuthChoice = serverConfigurationService.getBoolean("login.auth.choice",
// false);
// URL helperUrl = null;
// // /portal/relogin doesn't explicitly set a HELPER_DONE_URL so
// // we can't be sure it's there.
...
//
// if (StringUtils.equalsIgnoreCase(authPreferred,
// AuthChoices.CONTAINER.toString())) {
// log.debug("Going straight to container login");
// showAuthChoice = false;
// }
//
// if (showAuthChoice && !(StringUtils.isEmpty(helperPath) ||
...
// if (portalUrl != null && portalUrl.indexOf("/site/") < 1 &&
// portalUrl.startsWith(actualPortal)) {
...
// // Do we need to log this one? You can't really brute force
...
// }
//
// public void sendResponse(LoginRenderContext rcontext, HttpServletResponse
// res, String template, String contentType)
// throws IOException {
// // headers
// if (contentType == null) {
...
// }
// res.addDateHeader("Expires", System.currentTimeMillis() - (1000L * 60L * 60L
// * 24L * 365L));
...
// }
//
// public LoginRenderContext startPageContext(String skin, HttpServletRequest
// request, HttpServletResponse response) {
// LoginRenderEngine rengine = loginService.getRenderEngine(loginContext,
// request);
...
// }
//
// public LoginRenderContext startChoiceContext(String skin, HttpServletRequest
// request,
...
// rcontext.put("xloginChoiceIcon",
// serverConfigurationService.getString("xlogin.choice.icon"));
// // the URLs for these are set above, as containerLoginUrl and xloginUrl
//
// rcontext.put("containerLoginChoiceText",
// serverConfigurationService.getString("container.login.choice.text",
// serverConfigurationService.getString("login.text")));
// rcontext.put("xloginChoiceText",
// serverConfigurationService.getString("xlogin.choice.text",
...
// * @throws IOException
// */
// protected void complete(String returnUrl, Session session, Tool tool,
...
//
// /**
// * Gets the password reset URL. If looks for a configured URL, otherwise it
// * looks for the password reset tool in the gateway site and builds a link
// * to that.
// *
// * @return The password reset URL or <code>null</code> if there isn't one or
// * we can't find the password reset tool.
// */
// protected String getPasswordResetUrl() {
// // Has a password reset url been specified in sakai.properties? If so,
// // it rules.
// String passwordResetUrl =
// serverConfigurationService.getString("login.password.reset.url", null);
//
// if (passwordResetUrl == null) {
// // No explicit password reset url. Try and locate the tool on the
// // gateway page.
...
// ToolConfiguration resetTC =
// gatewaySite.getToolForCommonId("sakai.resetpass");
// if (resetTC != null) {
// passwordResetUrl = resetTC.getContainingPage().getUrl();
// }
// } catch (IdUnusedException iue) {
// log.warn(
// "No " + gatewaySiteId + " site found whilst building password reset url, set
// password.reset.url"
...
// *
// * Note that this could easily be extedned to track login
// * attempts per session and report on it here
// */
